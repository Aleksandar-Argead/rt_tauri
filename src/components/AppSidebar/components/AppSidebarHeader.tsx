...
export const AppSidebarHeader = ({
  ...
  const connectionEntries = React.useMemo(
    () => Object.entries(connections),
    [connections],
  );

  const activeConnection = React.useMemo(
    () => (activeConnectionId ? connections[activeConnectionId] : null),
                ...
                {activeConnection ? (
                  <>
                    <div className="flex aspect-square size-8 items-center justify-center rounded-lg bg-sidebar-primary text-sidebar-primary-foreground">
                      {String(activeConnection.platform)
                        .toLocaleLowerCase()
                        .trim() === "ios" && <Apple className="size-4" />}
                      {activeConnection.platform === "android" && (
                        <Bot className="size-4" />
                      )}
                      {activeConnection.platform === "web" && (
            ...
            >
              <DropdownMenuGroup>
                ...
                {connectionEntries.map(([id, connection], index) => (
                  <DropdownMenuItem
                    key={id}
                    onClick={() => setActiveConnectionId(id)}
                    className="gap-2 p-2"
                  >
                    <div className="flex aspect-square size-8 items-center justify-center rounded-lg bg-sidebar-primary text-sidebar-primary-foreground">
                      ...
                      {connection.platform === "android" && (
                        <Bot className="size-4" />
                      )}
                      {connection.platform === "web" && (
                        <Globe className="size-4" />
                      )}
                    </div>
